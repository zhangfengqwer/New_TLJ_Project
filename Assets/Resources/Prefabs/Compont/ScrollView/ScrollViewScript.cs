...
public class ScrollViewScript : MonoBehaviour {
    ...
    {
        GameObject pre = Resources.Load("Prefabs/Compont/ScrollView/ScrollView") as GameObject;
    ...
    public void addItemEnd()
    {
        ...
        float contentLength = m_itemList.Count * m_itemSize.x + (m_itemList.Count - 1) * m_jiange;
        m_content.GetComponent<RectTransform>().sizeDelta = new Vector2(contentLength, m_listSize.y);
        ...
        for (int i = 0; i < m_itemList.Count; i++)
        {
            ...
            //float x = -m_itemSize.x / 2 - i * (m_itemSize.x + m_jiange);
            float x = m_itemSize.x / 2 + i * (m_itemSize.x + m_jiange);
            m_itemList[i].transform.localPosition = new Vector3(x, 0, 0);
        }
    }
